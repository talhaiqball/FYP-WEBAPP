...
export default function SupervisorSidebar() {
    ...
    useEffect(() => {
        if (userid) {
            axios.get(`http://localhost:3001/getSupervisorInfo/${userid}`)
                .then(response => {
                    console.log('Supervisor Info:', response.data);
                    if (response.data) {
                        setSupervisor(response.data);
                    } else {
                        console.error('Supervisor data not found in API response');
                    }
                })
                .catch(error => {
                        ...
                        <div className="mt-auto" style={{ position: "fixed", left: 0, bottom: 0, width: 251, backgroundColor: '#0496FF', borderTop: '1px solid #fff' }}>
                            <div className="dropdown open">
                                <Link className="text-decoration-none text-white dropdown-toggle p-3"
                                    ...
                                    aria-haspopup="true"
                                    aria-expanded="false">
                                    <i className='bi bi-person-circle'></i><span className='ms-3'>{supervisor.name}</span>
                                </Link>
