...
function ViewTemporarilyEnrolledStudents() {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch('http://localhost:3001/viewTemporaryStudentList');
            if (!response.ok) {
    ...
    const handleVerify = () => {
        navigate('/coordinator/VerifyTemporarilyEnrolledStudents')
    }
            ...
            <div className="shadow-lg p-3 mb-5 mt-5 bg-body rounded">
                <div className="row ">
                    <div className="mb-4 mt-2 text-gred text-center">
                        <h2><b>Temporarily Enrolled Students</b></h2>
                    </div>
                </div>
...
export default ViewTemporarilyEnrolledStudents;