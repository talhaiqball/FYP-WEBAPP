...
const Login = () => {
  ...
  const {updateUserId } = useContext(SupervisorContext);
  ...
  const handleLogin = (event) => {
    ...
    axios.post(`http://localhost:3001/signinSupervisor/${userid}`, { password })
      .then(res => {
        console.log(res.data)
        updateUserId(userid)
        navigate(`/supervisor/Dashboard/${userid}`, { state: { password: password } })
      }).catch(err => {
        console.log(err)
      ...
      <div className="shadow-lg p-5 col-md-7 mb-5 mt-5 bg-body rounded">
        ...
        <form onSubmit={handleLogin}>
          ...
          <div className="text-center mt-3">
            ...
            <div className="mt-3"> <Link to={`/supervisor/ResetPassword/${userid}`} className='btn-link' style={{ color: "#0496FF" }}>Forgot password?</Link></div>
          </div>
        </form>
      </div>
    </div>
